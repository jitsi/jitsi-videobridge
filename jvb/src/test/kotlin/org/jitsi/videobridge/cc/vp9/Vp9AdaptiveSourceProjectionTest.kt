...
class Vp9AdaptiveSourceProjectionTest {
    ...
    fun singlePacketProjectionTest() {
        ...
        Assert.assertTrue(context.accept(packetInfo, packet.temporalLayerIndex, targetIndex))
        context.rewriteRtp(packetInfo)
        Assert.assertEquals(10001, packet.sequenceNumber.toLong())
        Assert.assertEquals(1003000, packet.timestamp)
        Assert.assertEquals(0, packet.pictureId.toLong())
    ...
    private fun runInOrderTest(generator: Vp9PacketGenerator, targetTid: Int) {
        ...
        val initialState = RtpState(1, 10000, 1000000)
        val targetIndex = getIndex(0, 0, targetTid)
        val context = Vp9AdaptiveSourceProjectionContext(diagnosticContext, payloadType,
            initialState, logger)
        ...
        for (i in 0..99999) {
            val packetInfo = generator.nextPacket()
            val packet = packetInfo.packetAs<Vp9Packet>()
            val accepted = context.accept(packetInfo, packet.temporalLayerIndex, targetIndex)
            ...
            if (packet.temporalLayerIndex <= targetIndex) {
                ...
                context.rewriteRtp(packetInfo)
                Assert.assertEquals(expectedSeq.toLong(), packet.sequenceNumber.toLong())
                Assert.assertEquals(expectedTs, packet.timestamp)
                Assert.assertEquals(expectedPicId.toLong(), packet.pictureId.toLong())
                Assert.assertEquals(expectedTl0PicIdx.toLong(), packet.TL0PICIDX.toLong())
    ...
    private fun doRunOutOfOrderTest(generator: Vp9PacketGenerator, targetTid: Int, seed: Long) {
        ...
        for (i in 0..99999) {
            ...
            if (origSeq isOlderThan oldestValidSeq && !accepted) {
                 ...
                 */
            } else if (packet.temporalLayerIndex <= targetIndex) {
                Assert.assertTrue(accepted)
                ...
                Assert.assertEquals(origTl0PicIdx.toLong(), packet.TL0PICIDX.toLong())
    ...
    fun slightlyDelayedKeyframeTest() {
        ...
        for (i in 0..2) {
            val packetInfo = generator.nextPacket()
            val packet = packetInfo.packetAs<Vp9Packet>()
            Assert.assertFalse(context.accept(packetInfo, getIndex(0, 0, packet.temporalLayerIndex), targetIndex))
        }
        Assert.assertTrue(context.accept(firstPacketInfo, getIndex(0, 0, firstPacket.temporalLayerIndex), targetIndex))
        context.rewriteRtp(firstPacketInfo)
        for (i in 0..9995) {
            val packetInfo = generator.nextPacket()
            val packet = packetInfo.packetAs<Vp9Packet>()
            Assert.assertTrue(context.accept(packetInfo, getIndex(0, 0, packet.temporalLayerIndex), targetIndex))
            context.rewriteRtp(packetInfo)
        }
    }
    ...
    fun veryDelayedKeyframeTest() {
        ...
        Assert.assertFalse(context.accept(firstPacketInfo, getIndex(0, 0, firstPacket.temporalLayerIndex), targetIndex))
        for (i in 0..9) {
            val packetInfo = generator.nextPacket()
            val packet = packetInfo.packetAs<Vp9Packet>()
            Assert.assertFalse(context.accept(packetInfo, getIndex(0, 0, packet.temporalLayerIndex), targetIndex))
    ...
    fun twoStreamsNoSwitchingTest() {
        ...
        for (i in 0..9999) {
            ...
            Assert.assertTrue(context.accept(packetInfo1, getIndex(1, 0, packet1.temporalLayerIndex), targetIndex))
            val packetInfo2 = generator2.nextPacket()
            val packet2 = packetInfo2.packetAs<Vp9Packet>()
            Assert.assertFalse(context.accept(packetInfo2, getIndex(0, 0, packet2.temporalLayerIndex), targetIndex))
            context.rewriteRtp(packetInfo1)
            Assert.assertEquals(expectedSeq.toLong(), packet1.sequenceNumber.toLong())
    ...
    fun twoStreamsSwitchingTest() {
        ...
        /* Start by wanting spatial layer 0 */for (i in 0..899) {
            ...
            Assert.assertTrue(context.accept(packetInfo1, getIndex(0, 0, packet1.temporalLayerIndex), targetIndex))
            context.rewriteRtp(packetInfo1)
            ...
            val packetInfo2 = generator2.nextPacket()
            val packet2 = packetInfo2.packetAs<Vp9Packet>()
            Assert.assertFalse(context.accept(packetInfo2, getIndex(1, 0, packet2.temporalLayerIndex), targetIndex))
            Assert.assertFalse(context.rewriteRtcp(srPacket2))
            Assert.assertEquals(expectedSeq.toLong(), packet1.sequenceNumber.toLong())
            Assert.assertEquals(expectedTs, packet1.timestamp)
            Assert.assertEquals(expectedPicId.toLong(), packet1.pictureId.toLong())
            Assert.assertEquals(expectedTl0PicIdx.toLong(), packet1.TL0PICIDX.toLong())
            expectedSeq = RtpUtils.applySequenceNumberDelta(expectedSeq, 1)
            if (packet1.isEndOfFrame) {
                expectedTs = RtpUtils.applyTimestampDelta(expectedTs, 3000)
                expectedPicId = applyExtendedPictureIdDelta(expectedPicId, 1)
        ...
        for (i in 0..89) {
            ...
            Assert.assertTrue(context.accept(packetInfo1, getIndex(0, 0, packet1.temporalLayerIndex),
                targetIndex))
            context.rewriteRtp(packetInfo1)
            Assert.assertTrue(context.rewriteRtcp(srPacket1))
            Assert.assertEquals(packet1.ssrc, srPacket1.senderSsrc)
            ...
            val packetInfo2 = generator2.nextPacket()
            val packet2 = packetInfo2.packetAs<Vp9Packet>()
            Assert.assertFalse(context.accept(packetInfo2, getIndex(1, 0, packet2.temporalLayerIndex),
                targetIndex))
            Assert.assertFalse(context.rewriteRtcp(srPacket2))
            Assert.assertEquals(expectedSeq.toLong(), packet1.sequenceNumber.toLong())
            Assert.assertEquals(expectedTs, packet1.timestamp)
        ...
        /* After a keyframe we should accept spatial layer 1 */for (i in 0..8999) {
            ...
            Assert.assertEquals(i == 0,
                context.accept(packetInfo1, getIndex(0, 0, packet1.temporalLayerIndex), targetIndex))
            ...
            Assert.assertTrue(context.accept(packetInfo2, getIndex(1, 0, packet2.temporalLayerIndex), targetIndex))
            ...
            Assert.assertEquals(expectedSeq.toLong(), packet2.sequenceNumber.toLong())
            Assert.assertEquals(expectedTs, packet2.timestamp)
            Assert.assertEquals(expectedPicId.toLong(), packet2.pictureId.toLong())
            Assert.assertEquals(expectedTl0PicIdx.toLong(), packet2.TL0PICIDX.toLong())
            expectedSeq = RtpUtils.applySequenceNumberDelta(expectedSeq, 1)
    ...
    fun temporalLayerSwitchingTest() {
        ...
        for (i in 0..9999) {
            val packetInfo = generator.nextPacket()
            val packet = packetInfo.packetAs<Vp9Packet>()
            val accepted = context.accept(packetInfo, getIndex(0, 0, packet.temporalLayerIndex), targetIndex)
            if (packet.isStartOfFrame && packet.temporalLayerIndex == 0) {
                expectedTl0PicIdx = applyTl0PicIdxDelta(expectedTl0PicIdx, 1)
            }
            if (accepted) {
    ...
    private fun runLargeDropoutTest(generator: Vp9PacketGenerator, targetIndex: Int) {
        ...
        while (gap < 65536) {
            ...
            /* Allow any values after a gap. */expectedSeq = RtpUtils.applySequenceNumberDelta(packet.sequenceNumber, 1)
    ...
    private class Vp9PacketGenerator internal constructor(val packetsPerFrame: Int) {
        private var seq = 0
        var ts: Long = 0
        private var picId = 0
        ...
        fun nextPacket(): PacketInfo {
            ...
            /*
            DePacketizer.VP9PayloadDescriptor.setStartOfPartition(rtpPacket.buffer,
                rtpPacket.payloadOffset, startOfFrame)
            Assert.assertTrue(DePacketizer.VP9PayloadDescriptor.setTemporalLayerIndex(rtpPacket.buffer,
                rtpPacket.payloadOffset, rtpPacket.payloadLength, tid))
            if (startOfFrame) {
                val szVP9PayloadDescriptor =
                    DePacketizer.VP9PayloadDescriptor.getSize(rtpPacket.buffer,
                        rtpPacket.payloadOffset, rtpPacket.payloadLength)
                DePacketizer.VP9PayloadHeader.setKeyFrame(rtpPacket.buffer,
                    rtpPacket.payloadOffset + szVP9PayloadDescriptor, keyframe)
            }
            */

            rtpPacket.isMarked = endOfFrame
            val vp9Packet = rtpPacket.toOtherType {
                buffer: ByteArray, offset: Int, length: Int -> Vp9Packet(buffer, offset, length)
            }

            /* Make sure our manipulations of the raw buffer were correct. */
            Assert.assertEquals(startOfFrame, vp9Packet.isStartOfFrame)
            Assert.assertEquals(tid.toLong(), vp9Packet.temporalLayerIndex.toLong())
            if (startOfFrame) {
                Assert.assertEquals(keyframe, vp9Packet.isKeyframe)
            }
        ...
        companion object {
            private val vp9PacketTemplate = DatatypeConverter.parseHexBinary( /* RTP Header */
                "80" + /* V, P, X, CC */
                    ...
                    "0000" + /* Seq */
                    "00000000" + /* TS */
                    "cafebabe" + /* SSRC */ /* VP9 Payload descriptor */
                    "90" + /* First byte, X, S set, PID = 0 */
                    "e0" + /* X byte, I, L, T set */
                    "8000" + /* I byte (ext pic id), M set */
                    "00" + /* L byte (tl0 pic idx) */
                    "00" + /* T/K byte (tid) */ /* VP9 payload header */
                    "00" + /* P = 0. */ /* Rest of payload is "don't care" for this code, except
