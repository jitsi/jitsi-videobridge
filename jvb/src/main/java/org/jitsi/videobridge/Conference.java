     ...
     implements AbstractEndpointMessageTransport.EndpointMessageTransportEventHandler
{
    ...
    private void handleMediaMessage(MediaEvent mediaEvent)
    {
        try
        {
            ...
            packetInfo.setPayloadType(new OpusPayloadType((byte) 111));

            // With a single endpoint, don't set the ID so that the translation is echoed back.
            if (getEndpointCount() > 1)
            {
                packetInfo.setEndpointId(tag.substring(0, 8));
            }
