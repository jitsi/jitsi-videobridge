            ...
            if (!recv.hasDeltas) {
                ...
                if (state.valid) {
                    if (recv.state.valid) {
                        ...
                        codecDeltas = if (state.codecState is Av1DDCodecState &&
                            recv.state.codecState !is Av1DDCodecState &&
                            recv.state.av1PersistentState != null
                        ) {
                            state.codecState?.getDeltas(recv.state.av1PersistentState)
                        } else {
                            state.codecState?.getDeltas(recv.state.codecState)
                        }
                    } else {
