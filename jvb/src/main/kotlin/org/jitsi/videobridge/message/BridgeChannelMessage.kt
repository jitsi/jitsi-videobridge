...
@JsonSubTypes(
    ...
    JsonSubTypes.Type(
        value = SyntheticSourceSendingChangeEvent::class,
        name = SyntheticSourceSendingChangeEvent.TYPE
    )
)
...
open class MessageHandler {
    ...
    fun handleMessage(message: BridgeChannelMessage): BridgeChannelMessage? {
        ...
        return when (message) {
            ...
            is SyntheticSourceSendingChangeEvent -> syntheticSourceSendingChange(message)
        }
    }
    ...
    open fun syntheticSourceSendingChange(message: SyntheticSourceSendingChangeEvent) =
...
/**
 * A message sent from the bridge to a client indicating that a synthetic source (audio generated by the bridge
 * rather than sent by an endpoint, e.g. translated audio) started or stopped sending. It is derived from the
 * `start`/`stop` mediajson events a translator sends to bracket a "talk". One source per message; the flat shape
 * matches lib-jitsi-meet's `BridgeChannel` validation of this colibriClass.
 */
class SyntheticSourceSendingChangeEvent(
    /** The name of the synthetic source whose sending state changed (e.g. "55555555-a0.hi"). */
    val sourceName: String,
    /** True if the source started sending, false if it stopped. */
    val sending: Boolean,
    /**
     * The RTP timestamp (in the source's media clock, i.e. 48000 Hz for Opus) at which the change takes effect, on
     * the same timeline as the source's media, so a receiver can align it with the audio it receives. A 32-bit value
     * that wraps at 2^32 (lib-jitsi-meet validates 0..0xFFFFFFFF). For a start it is the first packet's timestamp;
     * for a stop it is one past the last packet (the timestamp the next contiguous packet would carry), so a sending
     * run spans [start, stop).
     */
    val timestamp: Long
) : BridgeChannelMessage() {
    companion object {
        const val TYPE = "SyntheticSourceSendingChangeEvent"
    }
}
