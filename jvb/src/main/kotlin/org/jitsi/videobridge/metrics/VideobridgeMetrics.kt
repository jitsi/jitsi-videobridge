...
object VideobridgeMetrics {
    ...
    @JvmField
    val keyframeRequestsDroppedByBudget = metricsContainer.registerCounter(
        "keyframe_requests_dropped_by_budget",
        "Number of keyframe requests dropped specifically because the keyframe budget (jmt.keyframe.budget) " +
            "lengthened the source-wide interval beyond its configured floor (updated on endpoint/relay expiration)."
    )

    @JvmField
    val keyframeRequestsDroppedByBudgetApi = metricsContainer.registerCounter(
        "keyframe_requests_dropped_by_budget_api",
        "The subset of keyframe_requests_dropped_by_budget generated by the bridge itself, such as a layer " +
            "switch needing a keyframe, rather than forwarded from a receiver's own PLI/FIR."
    )

    @JvmField
    val keyframeBudgetWaits = metricsContainer.registerCounter(
        "keyframe_budget_waits",
        "Number of times a receiver waited for a keyframe request the budget had delayed, counted once per " +
            "receiver waiting even when one request satisfies several (updated on endpoint/relay expiration)."
    )

    @JvmField
    val keyframeBudgetWaitsApi = metricsContainer.registerCounter(
        "keyframe_budget_waits_api",
        "The subset of keyframe_budget_waits where the delayed request was generated by the bridge itself."
    )

    @JvmField
    val keyframeBudgetWaitMillisecondsTotal = metricsContainer.registerCounter(
        "keyframe_budget_wait_milliseconds_total",
        "Total milliseconds spent waiting for a keyframe request the budget had delayed, summed across every " +
            "receiver waiting (updated on endpoint/relay expiration). Divide by keyframe_budget_waits for the mean."
    )

    @JvmField
    val keyframeBudgetWaitMillisecondsTotalApi = metricsContainer.registerCounter(
        // Not "..._total_api": registerCounter appends "_total" only when the name doesn't already end with it,
        // so "..._total_api" would come out as "..._total_api_total" instead of ending in the unit suffix.
        "keyframe_budget_wait_milliseconds_api",
        "The subset of keyframe_budget_wait_milliseconds_total where the delayed request was generated by the " +
            "bridge itself."
    )

    @JvmField
    val layerSwitchKeyframeWaitsCompleted = metricsContainer.registerCounter(
        "layer_switch_keyframe_waits_completed",
        "Number of times an adaptive source projection finished waiting for the keyframe needed to complete a " +
            "layer switch, regardless of what governed the request's timing."
    )

    @JvmField
    val layerSwitchKeyframeWaitMillisecondsTotal = metricsContainer.registerCounter(
        "layer_switch_keyframe_wait_milliseconds_total",
        "Total milliseconds spent forwarding a stale layer while waiting for the keyframe needed to complete a " +
            "layer switch, summed across all switches. Divide by layer_switch_keyframe_waits_completed for the mean."
    )

    @JvmField
    val layerSwitchKeyframeWaitsInProgress = metricsContainer.registerLongGauge(
        "layer_switch_keyframe_waits_in_progress",
        "Number of adaptive source projections currently stuck forwarding a stale layer while waiting for the " +
            "keyframe needed to complete a layer switch."
    )
