...
object VideobridgeMetrics {
    ...
    @JvmField
    val keyframeBudgetWaitApiMillisecondsTotal = metricsContainer.registerCounter(
        // The unit ("milliseconds") goes last, immediately before the "_total" registerCounter appends, matching
        // keyframe_budget_wait_milliseconds_total above; "..._milliseconds_api" would come out with "_api" after
        // the unit instead.
        "keyframe_budget_wait_api_milliseconds",
        "The subset of keyframe_budget_wait_milliseconds_total where the delayed request was generated by the " +
            "bridge itself."
    )

    @JvmField
    val keyframeWaitsCompleted = metricsContainer.registerCounter(
        "keyframe_waits_completed",
        "Number of times an adaptive source projection finished waiting for a keyframe while it was not " +
            "suspended, whether to start forwarding for the first time, to resume after a suspension, or to " +
            "complete a mid-stream layer switch."
    )

    @JvmField
    val keyframeWaitMillisecondsTotal = metricsContainer.registerCounter(
        "keyframe_wait_milliseconds_total",
        "Total milliseconds spent forwarding a stale or absent layer while waiting for a keyframe (see " +
            "keyframe_waits_completed for what counts), summed across every wait. Divide by " +
            "keyframe_waits_completed for the mean."
    )
