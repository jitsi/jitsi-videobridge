    ...
    init {
        "Creating a delta via the factory" {
            ...
            "from a valid sixteen-bit buffer" {
                should("parse it correctly") {
                    ReceiveDelta.parse(
                        org.jitsi.rtp.extensions.bytearray.byteArrayOf(0x00, 0x28),
                        0,
            ...
            "from an invalid buffer" {
                should("throw an exception") {
                    shouldThrow<Exception> {
                        ReceiveDelta.parse(org.jitsi.rtp.extensions.bytearray.byteArrayOf(0x00, 0x28, 0x29), 0, 3)
                    }
                }
            }
        }
...
//                    }
            ...
            "when created from a buffer" {
                val buf = byteArrayOf(0x28)
                val eightBitReceiveDelta = EightBitReceiveDelta.fromBuffer(buf, 0)
...
//                    }
            ...
            "when created from a buffer" {
                val buf = org.jitsi.rtp.extensions.bytearray.byteArrayOf(0x00, 0x28, 0xDE, 0xAD)
                val sixteenBitReceiveDelta = SixteenBitReceiveDelta.fromBuffer(buf, 0)
                should("parse the value correctly") {
                    sixteenBitReceiveDelta.deltaMs shouldBe 10.0
                }
