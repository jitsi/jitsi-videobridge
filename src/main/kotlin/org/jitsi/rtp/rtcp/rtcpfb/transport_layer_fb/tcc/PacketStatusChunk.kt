...
internal abstract class PacketStatusChunk : Iterable<PacketStatusSymbol> {
    ...
    companion object {
        ...
        fun getChunkType(buf: ByteArray, offset: Int): PacketStatusChunkType =
            PacketStatusChunkType.fromInt(buf.get(offset).getBit(0))
        fun setChunkType(buf: ByteArray, offset: Int, chunkType: PacketStatusChunkType) =
            buf.putNumberAsBits(offset, 0, 1, chunkType.value.toByte())
         ...
         */
        fun parse(buf: ByteArray, offset: Int): PacketStatusChunk {
            return when (getChunkType(buf, offset)) {
                PacketStatusChunkType.RUN_LENGTH_CHUNK -> RunLengthChunk.fromBuffer(buf, offset)
                PacketStatusChunkType.STATUS_VECTOR_CHUNK -> StatusVectorChunk.fromBuffer(buf, offset)
                else -> throw Exception("Unrecognized packet status chunk type: ${getChunkType(buf, offset)}")
            }
        }
    }
}
...
internal class RunLengthChunk(
    ...
    override fun writeTo(buf: ByteArray, offset: Int) {
        setChunkType(buf, offset, PacketStatusChunkType.RUN_LENGTH_CHUNK)
        setStatusSymbol(buf, offset, statusSymbol)
        setRunLength(buf, offset, runLength)
    }

    companion object {
        // Offset is to the start of this chunk
        fun fromBuffer(buf: ByteArray, offset: Int): RunLengthChunk {
            val statusSymbol = getStatusSymbol(buf, offset)
            val runLength = getRunLength(buf, offset)

            return RunLengthChunk(statusSymbol, runLength)
        }

        fun getStatusSymbol(buf: ByteArray, offset: Int): PacketStatusSymbol =
            TwoBitPacketStatusSymbol.fromInt(buf.getBitsAsInt(offset, 1, 2))
        fun setStatusSymbol(buf: ByteArray, offset: Int, statusSymbol: PacketStatusSymbol) {
            buf.putNumberAsBits(offset, 1, 2, statusSymbol.value.toByte())
        }

        fun getRunLength(buf: ByteArray, offset: Int): Int {
            val byte0 = buf.get(offset) and 0x1F.toByte()
            val byte1 = buf.get(offset + 1)

            return (byte0.toPositiveInt() shl 8) or (byte1.toPositiveInt())
        }
        fun setRunLength(buf: ByteArray, offset: Int, runLength: Int) {
            ...
            buf.set(offset, byte0val)
            buf.set(offset + 1, byte1)
        }
    }
}
...
internal class StatusVectorChunk(
    ...
    override fun writeTo(buf: ByteArray, offset: Int) {
        setChunkType(buf, offset, PacketStatusChunkType.STATUS_VECTOR_CHUNK)
        setSymbolSizeBits(buf, offset, symbolSizeBits)
        setSymbolList(buf, offset, packetStatusSymbols, symbolSizeBits)
    }

    companion object {
        fun fromBuffer(buf: ByteArray, offset: Int): StatusVectorChunk {
            val symbolSizeBits = getSymbolsSizeBits(buf, offset)
            val packetStatusSymbols = getSymbolList(buf, offset)
            return StatusVectorChunk(symbolSizeBits, packetStatusSymbols)
        }
        fun getSymbolsSizeBits(buf: ByteArray, offset: Int): Int {
            val symbolSizeBit = buf.get(offset).getBits(1, 1).toPositiveInt()
        ...
        fun setSymbolSizeBits(buf: ByteArray, offset: Int, symbolSizeBits: Int) {
            val symbolSizeBit = when(symbolSizeBits) {
                1 -> 0
                2 -> 1
                else -> throw Exception("Unsupported symbol size: $symbolSizeBits bits")
            }
            buf.putBits(offset, 1, symbolSizeBit.toByte(), 1)
        }

        fun getSymbolList(buf: ByteArray, offset: Int): List<PacketStatusSymbol> {
            val symbolsSize = getSymbolsSizeBits(buf, offset)
            ...
            for (bitIndex in 2..15 step symbolsSize) {
                ...
                val symbol = when (symbolsSize) {
                    1 -> OneBitPacketStatusSymbol.fromInt((buf.get(offset + currByte).getBits(bitInByteIndex, symbolsSize)).toPositiveInt())
                    2 -> TwoBitPacketStatusSymbol.fromInt((buf.get(offset + currByte).getBits(bitInByteIndex, symbolsSize)).toPositiveInt())
                    else -> throw Exception("Invalid StatusVectorChunk symbol size ($symbolsSize bits)")
                }
                symbols.add(symbol)
            }
            return symbols
        }
        fun setSymbolList(buf: ByteArray, offset: Int, statusSymbols: List<PacketStatusSymbol>, symbolSizeBits: Int) {
            for (i in 0 until statusSymbols.size) {
                ...
                val bitInByteIndex = if (bitIndex <= 7) bitIndex else bitIndex - 8
                buf.putBits(offset + byteIndex, bitInByteIndex, symbol.value.toByte(), symbolSizeBits)
            }
        }
    }
}
