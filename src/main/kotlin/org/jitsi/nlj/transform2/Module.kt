...
class ModuleChainBuilder {
    companion object {
        fun build(f: ModuleChain.() -> Unit): ModuleChain {
            val chain = ModuleChain()
            chain.f()
            return chain
        }
...
abstract class Module(var name: String, protected val debug: Boolean = false) {
    protected var nextModule: NextModule? = null
    ...
    open fun attach(nextModule: NextModule) {
    ...
    abstract protected fun doProcessPackets(p: List<Packet>)
...
/*abstract*/ class SplitterModule: Module("") {
    ...
    fun addSubChain(predicate: PacketPredicate, nextModule: ModuleChain) {
        transformPaths[predicate] = nextModule
    }
    ...
    override fun attach(nextModule: NextModule) = throw Exception()
