...
//                break;
     ...
     */
    public void processPacketAESCM(SrtcpPacket pkt, int index)
    {
//        int ssrc = (int) pkt.getHeader().getSenderSsrc();
        ...
        */
        ...
        int payloadOffset = 8;
        int payloadLength = pkt.getSize() - payloadOffset;

        cipherCtr.process(
                pkt.getBuf().array(), payloadOffset, payloadLength,
                ivStore);
    }
     ...
     * @param pkt the RTP packet to be encrypted/decrypted
     */
    public void processPacketAESF8(SrtcpPacket pkt, int index)
    {
    ...
    private static int getSRTCPIndex(RtcpPacket pkt, int authTagLen)
    {
        int offset = pkt.getSize() - (4 + authTagLen);
        return pkt.getBuf().getInt(offset);
    }
     ...
     */
    synchronized public boolean reverseTransformPacket(SrtcpPacket pkt)
    {
        int tagLength = policy.getAuthTagLength();
//        int indexEflag = getSRTCPIndex(pkt, tagLength);
        pkt.getAuthTag(tagLength);

//        if ((indexEflag & 0x80000000) == 0x80000000)
//            decrypt = true;
        boolean decrypt = pkt.isEncrypted(tagLength);
...
//        int index = indexEflag & ~0x80000000;
        ...
        if (policy.getAuthType() != SRTPPolicy.NULL_AUTHENTICATION)
        {
            ...
            ByteBuffer authData = (ByteBuffer)pkt.getBuf().duplicate().position(pkt.getSize() - tagLength).limit(tagLength);

            // Shrink packet to remove the authentication tag and index
            // because this is part of authenicated data
            pkt.getBuf().limit(pkt.getBuf().limit() - (tagLength + 4));

            // compute, then save authentication in tagStore
            //TODO: it looks like this used to pass the entire eflag/index field (without the eflag stripped)
            // is that right?
//            authenticatePacketHMAC(pkt.getBuf().array(), 0, pkt.getSize(), indexEflag);
            authenticatePacketHMAC(pkt.getBuf().array(), 0, pkt.getSize(), index);

            // compare authentication tags using constant time comparison
            int nonEqual = 0;
            for (int i = 0; i < tagLength; i++)
            {
                nonEqual |= (authData.get(i) ^ tagStore[i]);
            }
            if (nonEqual != 0)
                return false;
        }
     ...
     */
    synchronized public void transformPacket(SrtcpPacket pkt)
    {
        //TODO
        throw new NotImplementedException();
...
//        //TODO: I don't think we have to 'grow' the packet, just re-set its limit
//        // (for now we'll assume all packet buffers have the space for the tag.  will
//        // fix it the first time we hit an array exception)
