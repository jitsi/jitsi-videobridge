 ...
 */
public class LastNTransformEngine
    ...
    public RawPacket[] reverseTransform(RawPacket[] pkts)
    {
        ...
        else
        {
            for (int i = 0; i < pkts.length; i++)
            {
                ...
                RTPEncodingImpl encoding = videoChannel
                    .getMediaStreamTrackReceiver().resolveRTPEncoding(oldPkt);

                if (oldPkt != null
                    && (encoding == null || !encoding.isForwarded()))
                {
                    RawPacket newPkt = reverseTransform(oldPkt);
