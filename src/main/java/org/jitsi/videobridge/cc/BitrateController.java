...
public class BitrateController
{
     ...
     * changes greater than {@link CcConfig#bweChangeThresholdPct}/100 of the last bandwidth
     * estimation.
     */
    ...
    )
    {
        ...
        trustBwe = CcConfig.trustBwe.get();
     ...
     * percent) {@link CcConfig#bweChangeThresholdPct} with respect to the previous
        ...
        long previousBwe, long currentBwe)
    {
        ...
        return deltaBwe > 0
            ||  deltaBwe < -1 * previousBwe * CcConfig.bweChangeThresholdPct.get() / 100;
    }
    ...
    private class TrackBitrateAllocation
    {
            ...
            boolean fitsInLastN, boolean selected, int maxFrameHeight)
        {
            ...
            for (RTPEncodingDesc encoding : encodings)
            {
                ...
                if (selected)
                {
                    ...
                    if (encoding.getHeight() < CcConfig.onstagePreferredHeight.get()
                        || encoding.getFrameRate() >= CcConfig.onstagePreferredFramerate.get())
                    {
                ...
                else if (encoding.getHeight() <= CcConfig.thumbnailMaxHeightPx.get())
                {
        ...
        private void improve(long maxBps)
        {
            ...
            {
                if (!CcConfig.onstageVideoSuspensionEnabled.get())
                {
                    ratedTargetIdx = 0;
