...
public class RtpChannel
  ...
  /**
   * The <tt>Logger</tt> used by the <tt>RtpChannel</tt> class and its instances
  ...
  /**
   * The speech activity of the <tt>Endpoint</tt>s in the multipoint conference
   * in which this <tt>Channel</tt> is participating.
   */
  protected final ConferenceSpeechActivity conferenceSpeechActivity;

  /**
   * Holds the <tt>RtpChannelDatagramFilter</tt> instances (if any) used by this
   * channel. The filter for RTP is at index 0, the filter for RTCP at index 1.
   */
  ...
  /**
   * The type of RTP-level relay (in the terms specified by RFC 3550 &quot;RTP:
   * A Transport Protocol for Real-Time Applications&quot; in section 2.3
   * &quot;Mixers and Translators&quot;) used for this
   * <tt>Channel</tt>.
   */
  ...
  /**
   * The <tt>PropertyChangeListener</tt> which listens to changes of the values
   * of {@link #stream}'s properties.
   */
   ...
   * <tt>RtpChannel</tt> is to be a part of (or <tt>null</tt> if no it is not to
   ...
   * control <tt>DatagramSocket</tt> of this <tt>Channel</tt> is to be accepted
   * for further processing within Jitsi Videobridge or is to be
   * rejected/dropped. When the first <tt>DatagramPacket</tt> arrives, makes its
   * source the only acceptable one for the control
   ...
   * acceptable one for the data <tt>DatagramSocket</tt>
   ...
   * @return <tt>true</tt> if <tt>receiveSSRC</tt> was added to the list (i.e.
   * was not previously there); otherwise, <tt>false</tt>
   */
   ...
   * properties of this instance. Thus, the specified <tt>iq</tt>
   ...
   * identified a speaker switch event in the multipoint conference and there is
   * now a new dominant speaker.
   */
   ...
   * <tt>rtcp</tt> is false) or RTCP (if <tt>rtcp</tt> is true) packets for this
   * <tt>RtpChannel</tt>.
   *
   ...
   * anywhere else. This should probably be refactored.
   *
   * @return the number of packets that are sent or received since last time the
   * method is called.
   */
  ...
  /**
   * Returns the number of received bytes since the last time the method was
   * called.
   *
   * Note: {@link org.jitsi.videobridge.stats.VideobridgeStatistics} uses this
   * method and it relies on the fact that the method is not called from
  ...
  /**
   * Returns the number of sent bytes since the last time the method was called.
   *
   * Note: {@link org.jitsi.videobridge.stats.VideobridgeStatistics} uses this
   * method and it relies on the fact that the method is not called from
   * anywhere else. This should probably be refactored.
  ...
  /**
   * Gets the type of RTP-level relay (in the terms specified by RFC 3550 "RTP:
   * A Transport Protocol for Real-Time Applications" in section 2.3 "Mixers and
   ...
   * terms of Jitsi Videobridge and which adapts this <tt>Channel</tt>
   ...
   * this <tt>Channel</tt>.
   *
   * @param channel the <tt>Channel</tt> to be checked whether it is within the
   * set of <tt>Channel<tt>s limited by <tt>lastN</tt> i.e. whether its RTP
   * streams are to be sent to the remote endpoint of this
   * <tt>Channel</tt>
   * @return <tt>true</tt> if the RTP streams of <tt>channel</tt> are to be sent
   * to the remote endpoint of this <tt>Channel</tt>; otherwise,
   * <tt>false</tt>. The implementation of the <tt>RtpChannel</tt> class always
   * returns <tt>true</tt>.
   */
   ...
   * <tt>SctpConnection</tt> has become ready i.e. connected to the remote peer
   * and operational.
   *
   ...
   * the payload types (i.e. the <tt>MediaFormat</tt>s) to be used by this
   * <tt>Channel</tt>
   */
  ...
  /**
   * Sets the type of RTP-level relay (in the terms specified by RFC 3550 "RTP:
   * A Transport Protocol for Real-Time Applications" in section 2.3 "Mixers and
   ...
   * @return a list of the <tt>Endpoint</tt>s which should be asked for (video)
   * keyframes because, for example, they are entering the set of
   ...
   *
   * @param adaptiveLastN <tt>true</tt> to enable and <tt>false</tt> to disable
   * adaptive lastN.
   */
